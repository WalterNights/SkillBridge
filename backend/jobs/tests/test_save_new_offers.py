...
def _data(url: str, title: str = 'A role') -> JobOfferData:
    return JobOfferData(
        title=title, company='Co', location='Remote',
        summary='Summary', url=url, keywords='python',
    )
...
class TestSaveNewOffers:
    ...
    def test_does_not_duplicate_existing_url(self):
        JobOffer.objects.create(
            ...
            summary='Old', url='https://example.com/a', keywords='python',
        )
    ...
    def test_partial_overlap_only_creates_new_ones(self):
        JobOffer.objects.create(
            title='Already there', company='X', location='X',
            summary='X', url='https://example.com/a', keywords='',
        )
