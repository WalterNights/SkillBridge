...
@Component({
  ...
  template: `
    <div class="space-y-2">
      ...
      <p *ngIf="hint && !error" class="text-xs text-gray-500 dark:text-dark-text-secondary">
        {{ hint }}
      </p>
    </div>
  `,
...
export class InputComponent implements ControlValueAccessor {
  ...
  getInputClasses(): string {
    const baseClasses =
      'block w-full py-3 bg-white dark:bg-dark-bg-tertiary border rounded-xl text-gray-900 dark:text-dark-text-primary placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:border-transparent transition-all duration-200';
